...
async function rebuild(): Promise<void> {
    ...
    try {
        ...
        await new Promise<void>((resolve, reject) => {
            build.on('exit', code => (code === 0 ? resolve() : reject(new Error(`tsc exited with ${code}`))));
        });

        const resources = spawn('bun', ['run', 'build:resources'], { cwd: root, stdio: 'inherit' });
        await new Promise<void>((resolve, reject) => {
            resources.on('exit', code => (code === 0 ? resolve() : reject(new Error(`build:resources exited with ${code}`))));
        });
...
watch(join(root, 'src'), { recursive: true }, (_event, filename) => {
    ...
    if (filename.endsWith('.ts') || filename.endsWith('.ui'))
        scheduleRebuild();
});
